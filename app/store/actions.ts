...
import { State, player, Coordinate } from '../types/app'
import { AnyAction } from '../lib'
import { getCoordValue } from './selectors'
...
export const checkWinner = (maxMoves: number): ThunkAction => (dispatch, getState) => {
  const state = getState()
  const marker = getCoordValue(state);
  const map = get.map(state)[marker];
  
  const lastCoords = map[map.length - 1];
  ...
  for (let fromY = from.y, fromX = from.x, countMarkers = 0; fromY <= to.y && fromX <= to.x; fromY += 1, fromX += 1) {
  ...
  for (let fromY = from.y, fromX = to.x, countMarkers = 0; fromY <= to.y && fromX >= from.x; fromY += 1, fromX -= 1) {
    if (map[getHashKey(fromX, fromY)] === marker) {
      countMarkers += 1
    }
